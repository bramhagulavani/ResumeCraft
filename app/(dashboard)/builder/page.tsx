...
function BuilderPageInner() {
  ...
  const handleSkillChange = (i: number, v: string) => {
    const u = [...skills]; u[i] = v; setSkills(u);
  };
  const addSkill = () => setSkills([...skills, ""]);
  const removeSkill = (i: number) => setSkills(skills.filter((_, j) => j !== i));

  const handleExpChange = (i: number, field: keyof ResumeData["experience"][0], v: string) => {
    const u = [...experience]; u[i][field] = v; setExperience(u);
  };
  ...
  const handleEduChange = (i: number, field: keyof ResumeData["education"][0], v: string) => {
    const u = [...education]; u[i][field] = v; setEducation(u);
  };
  const addEdu = () => setEducation([...education, { college: "", degree: "", year: "" }]);
  const removeEdu = (i: number) => setEducation(education.filter((_, j) => j !== i));

  const handleProjChange = (i: number, field: keyof ResumeData["projects"][0], v: string) => {
    const u = [...projects]; u[i][field] = v; setProjects(u);
  };
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          {resumeId && (
            <p className="text-xs text-gray-400 dark:text-slate-500 mt-0.5">Editing existing resume</p>
          )}
        </div>
        <div className="flex gap-3">
          <button
            ...
            className="px-5 py-2 bg-gray-100 dark:bg-white/[0.06] hover:bg-gray-200 dark:hover:bg-white/[0.1] disabled:opacity-40 disabled:cursor-not-allowed rounded-xl text-sm font-semibold transition-all duration-300 text-gray-700 dark:text-slate-300 border border-gray-200 dark:border-white/[0.06]"
          >
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="px-5 py-2 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed rounded-xl text-sm font-semibold transition-all duration-300 text-white shadow-lg shadow-violet-900/30"
          >
      ...
      <div className="grid grid-cols-2 gap-6 flex-1 min-h-0">
        ...
        {/* LEFT: FORM */}
        <div className="bg-white dark:bg-white/[0.02] rounded-2xl border border-gray-200 dark:border-white/[0.06] overflow-y-auto">
        ...
        {/* RIGHT: LIVE PREVIEW */}
        <div className="rounded-2xl overflow-y-auto shadow-xl flex flex-col bg-slate-200 dark:bg-[#e2e8f0]">
      ...
      {aiModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 px-4">
          <div className="relative bg-white dark:bg-[#13131a] border border-gray-200 dark:border-white/[0.08] rounded-2xl p-8 w-full max-w-lg shadow-2xl shadow-violet-900/20 transition-colors duration-300">
            <h2 className="text-xl font-extrabold mb-2 tracking-tight text-gray-900 dark:text-white">
              🤖 Generate with AI
            </h2>
